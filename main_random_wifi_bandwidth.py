...
def plot_deadlines(statsdict):
    ...
    cloud.savefig("figs/random_wifi_bandwidth_deadlines.pdf",format='pdf',bbox_inches = 'tight')
...
def plot_load(statsdict):
    ...
    p6 = axes.bar(ind, C3, width, bottom=[D1[i]+E1[i]+E2[i]+C1[i]+C2[i] for i in range(0,9)],color='turquoise')
...
#cloud.tight_layout()
    cloud.savefig("figs/random_wifi_bandwidth_load.pdf",format='pdf',bbox_inches = 'tight')
