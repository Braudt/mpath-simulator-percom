...
def plot_deadlines(statsdict):
    ...
    axes.set_xlabel('WiFi Bandwidth Standard Deviation (%)', **axis_font)
    ...
    cloud.legend((plt1[0], plt2[0]), ("Tasks over deadline", "Avg time over deadline"), bbox_to_anchor=(1.1, 1.5),
                 fontsize=14)

    cloud.savefig(os.getcwd() + "/figs/random_wifi_bandwidth_deadlines.pdf", format='pdf', bbox_inches='tight')
...
def plot_load(statsdict):
    ...
    p3 = axes.bar(ind, E2, width, bottom=[D1[i] + E1[i] for i in range(0, 9)], color='mediumseagreen')
    p4 = axes.bar(ind, C1, width, bottom=[D1[i] + E1[i] + E2[i] for i in range(0, 9)], color='dodgerblue')
    p5 = axes.bar(ind, C2, width, bottom=[D1[i] + E1[i] + E2[i] + C1[i] for i in range(0, 9)], color='skyblue')
    p6 = axes.bar(ind, C3, width, bottom=[D1[i] + E1[i] + E2[i] + C1[i] + C2[i] for i in range(0, 9)],
                  color='turquoise')

    axes.legend((p1[0], p2[0], p3[0], p4[0], p5[0], p6[0]), ('D1', 'E1', 'E2', 'C1', 'C2', 'C3'), bbox_to_anchor=(1, 1),
                fontsize=14)
    axes.set_xlabel('WiFi Bandwdth Standard Deviation (%)', **axis_font)
    axes.set_ylabel('Tasks (x1000)', **axis_font)
    axes.set_xticks(ind)
    axes.set_xticklabels(['0', '12.5', '25', '37.5', '50', '62.5', '75', '87.5', '100'],
                         rotation=45, **axis_font)

    # cloud.tight_layout()
    cloud.savefig(os.getcwd() + "/figs/random_wifi_bandwidth_load.pdf", format='pdf', bbox_inches='tight')
...
def main():
    ...
    importname = "configs.config_random_wifi_bandwidth." + module

    cloud_lat = ["0", "125", "25", "375", "50", "625", "75", "875", "100"]
